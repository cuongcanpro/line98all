...
DragonBonesData* DBCCFactory::loadDragonBonesData(const std::string &dragonBonesFilePath, const std::string &name)
{
	...
	cocos2d::log("LoadDragonBonesData: %s", dragonBonesFilePath.c_str());
    const auto &data = cocos2d::FileUtils::getInstance()->getDataFromFile(dragonBonesFilePath);
    if (data.getSize() == 0)
    {
	//	CCAssert(false, "Load data of animation error!");
		cocos2d::log("%s", dragonBonesFilePath.c_str());
        return nullptr;
    }
	...
	std::string dataString = std::string((const char*)data.getBytes(), data.getSize());
    // load skeleton.xml using XML parser.
    XMLDocument doc;
	doc.Parse(dataString.c_str(), data.getSize());
    // paser dragonbones skeleton data.
    XMLDataParser parser;
    DragonBonesData *dragonBonesData = parser.parseDragonBonesData(doc.RootElement(), 1);
    addDragonBonesData(dragonBonesData, name);
    return dragonBonesData;
}

ITextureAtlas* DBCCFactory::loadTextureAtlas(const std::string &textureAtlasFile, const std::string &name, const std::string &textureFile)
{
	...
	{
		file_type = textureAtlasFile.substr(pos + 1);
		if (textureFile.empty()){
			file_png = textureAtlasFile.substr(0, pos);
			file_png = file_png + ".png";
		}
		else
		{
			file_png = textureFile;
		}
	}

	DBCCTextureAtlas *textureAtlas = new DBCCTextureAtlas();

	if (std::strcmp(file_type.c_str(), "plist") == 0 || std::strcmp(file_type.c_str(), "PLIST") == 0)
	{
		PlistDataParser parser;

		textureAtlas->textureAtlasData = parser.parseTextureAtlasFile(textureAtlasFile);
		textureAtlas->textureAtlasData->name = name;
		textureAtlas->textureAtlasData->imagePath = file_png;
	}
	else
	{
		...
		if (data.getSize() == 0)
		{
			cocos2d::log("Error DBCCFactory::loadTextureAtlas texture: %s", textureAtlasFile.c_str());
			return nullptr;
		}
		...
		pos = textureAtlasFile.find_last_of("/");
...
void* DBCCFactory::generateDisplay(const ITextureAtlas *textureAtlas, const TextureData *textureData, const DisplayData *displayData) const
{
	...
	{
		if (textureData){
			cocos2d::log("generateDisplay error: %s", textureData->name.c_str());
		}
		return nullptr;
	}
    ...
    cocos2d::Node *display = DBCCSprite::createWithSpriteFrame(spriteFrame);
	
	
    display->setCascadeColorEnabled(true);
    display->setCascadeOpacityEnabled(true);
    display->retain();
	display->setName(textureData->name);
	...
	float scale = cocos2d::Director::getInstance()->getContentScaleFactor();
	display->setAnchorPoint(cocos2d::Vec2(pivotX*scale / originSize.width, 1.f - pivotY*scale / originSize.height));
	display->setContentSize(cocos2d::Size(originSize.width / scale, originSize.height / scale));
    return display;
}
