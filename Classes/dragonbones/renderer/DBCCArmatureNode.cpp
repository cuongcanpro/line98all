...
void DBCCArmatureNode::loadingDataCallBack(const std::string &armatureName, bool result)
{
	if (!result)
	{
		return;
...
void DBCCArmatureNode::setCameraMask(unsigned short mask, bool applyChildren /* = true */)
{
	Node::setCameraMask(mask, applyChildren);
	setCameraMaskArmature(_armature, mask, applyChildren);
}
void DBCCArmatureNode::setCameraMaskArmature(Armature* _armature, unsigned short mask, bool applyChildren)
{
	const std::vector<Bone*> listBones = _armature->getBones();
	for (auto& object : listBones)
	{
		...
		for (auto& slot : listSlots)
		{
			...
			for (size_t i = 0; i < displayList.size(); i++)
			{
				if (displayList[i].second == DisplayType::DT_IMAGE){
					DBCCSprite* display = (DBCCSprite*)displayList[i].first;
					display->setCameraMask(mask, applyChildren);
				}
				else if (displayList[i].second == DisplayType::DT_ARMATURE)
				{
					Armature* armature = (Armature*)displayList[i].first;
					setCameraMaskArmature(armature, mask, applyChildren);
				}
			}
		}
	}
