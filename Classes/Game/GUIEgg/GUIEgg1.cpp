...
void GUIEgg::onButtonRelease(int buttonID, CCTouch* touch)
{
	...
	case BTN_DOWNLOAD:
	{
		JNIUtils::openApp("com.lightgames.eggShoot2017");
	}
	break;
	case BTN_BACK_ID:
		{
...
void GUIEgg::callBackSplash()
{
	...
	labelScore = LabelBM::create("Score: 0", FONT_GAME_TYPE);
	...
	labelHighscore = LabelBM::create("Highscore: 0", FONT_GAME_TYPE);
    labelHighscore->setScale(0.7f);
	...
	labelRow = LabelBM::create("10 ROWS", FONT_GAME_TYPE);
		...
		if (gameType == ENDLESS) {
			...
			if (speedY > 0 && genBall->isVisible()) {
				...
				if (distance <= WIDTH_BALL_EGG) {
					...
					group->runAction(CCSequence::create(CCEaseBackOut::create(CCMoveTo::create(0.7f, ccp(group->getPositionX(), group->getPositionY() + size.height * 0.1f)))
						, CCFadeOut::create(0.5), CCHide::create(), NULL));
...
void GUIEgg::addMoveBall(int row, int column) {
	...
	if (ball->checkEatBall()) {
		...
		if (gameType == ENDLESS || gameType == STOMP) {
			score = score + GroupScoreEgg::getNormalBonusScore(arrayEatBall.size(), countEat - arrayEatBall.size(), bonusEffect->getMul(), bonusScore);
...
void GUIEgg::showParticle(float px, float py, int id) {
	...
	effect->create(("particles/egg" + GameUtility::toString(1) + ".plist").c_str(), layer);
	string s = "480_800/lineBall_0_" + GameUtility::toString(id) + ".png";
...
void GUIEgg::updateTime() {
	...
	labelScore->setText(convertTime(countTime).c_str());
...
void GUIEgg::initDragonbone()
{
	...
	//	//effectLight->removeFromParent();
	//}
	/*DBCCFactory::getInstance()->loadDragonBonesData("animation/LightBg/skeleton.xml", "LightBg");
	DBCCFactory::getInstance()->loadTextureAtlas("animation/LightBg/texture.plist", "LightBg");
...
void GUIEgg::effectImage(int id)
{
	for (int j = 0; j < 10; j++) {
		...
		image->setTexture("480_800/lineBall_0_" + StringUtils::toString(id) + ".png");
		CCSize size = Director::getInstance()->getOpenGLView()->getVisibleSize();
